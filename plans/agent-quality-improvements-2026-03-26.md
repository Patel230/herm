...
- [ ] 2e: Improve `personality.md` template — replace "grumpy" affect with behavioral language: "Direct and opinionated. Skip pleasantries. If something is a bad idea, say so and suggest the better approach. Match the user's technical level." Ensure this only renders when personality is set (current behavior).
