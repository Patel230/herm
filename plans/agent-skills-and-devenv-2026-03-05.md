...
- [ ] 1a: Add `skills.go` — define `Skill` struct (`Name`, `Description`, `Content string`), `loadSkills(dir string) ([]Skill, error)` that reads all `.md` files from a directory, parses YAML front matter (name + description), and returns the skill list. Handle missing directory gracefully (return empty).
