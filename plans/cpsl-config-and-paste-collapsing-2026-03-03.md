...
- [x] 3d: Add tests for command parsing (recognizes `/config`, rejects unknown `/foo`, doesn't treat `/` in normal text as command), mode switching (enter/exit, Esc discards, Enter saves, Ctrl+C discards), validation rejection, and window resize in config mode.
