...
## Phase 11: Debounce sub-agent delta renders

**Problem:** When a sub-agent streams its final response (e.g. 2960 tokens over 30 seconds), each token generates an `EventSubAgentDelta` that calls `a.render()`. This produces ~3000 full renders in a burst. The done event (`EventSubAgentStatus "done"`) enters the channel AFTER all deltas, and the main agent's summary text enters AFTER the done event — strict FIFO ordering. But the TUI must churn through ~3000 delta renders before reaching the done event. If each render takes 3-5ms, that's 9-15 seconds of blocking the TUI event drain. During this window, the main agent (running in a separate goroutine) sees `state.done=true` via status checks, gets results, calls the LLM, and starts streaming summary text. The summary text deltas queue behind the done event in the channel, but the user sees "summary text with spinner" because the TUI is still working through the delta backlog and hasn't reached the done event yet.

**Fix:** Stop calling `a.render()` on every `EventSubAgentDelta`. Instead, just update `sa.status` silently. The 50ms `agentTickMsg` ticker already triggers renders via `handleResult` → `drainAgentEvents`. Sub-agent status snippets update at most 20 times/second (every tick), which is plenty for a text snippet display. Critical events (`EventSubAgentStatus "done"`, `EventSubAgentStart`, `EventTextDelta`) keep their immediate `render()` calls so completion checkmarks and main agent text appear without delay.

Also apply the same treatment to `EventSubAgentStatus` when `event.Text != "done"` (tool start notifications) — these are high-frequency display updates that don't need per-event renders.

- [x] 11a: In the `EventSubAgentDelta` handler (`agentui.go:571-582`), remove the `a.render()` call. The status snippet is still updated on `sa.status` — it just won't trigger an immediate render. The next `agentTickMsg` (every 50ms) will render it via the existing tick → `drainAgentEvents` → `render` path
- [x] 11b: In the `EventSubAgentStatus` handler (`agentui.go:584`), move the `a.render()` call inside the `if event.Text == "done"` branch and the `else` branch that handles errors. For the non-done branch (tool start notifications at line 612-617), remove the `a.render()` call — these are frequent display-only updates that the tick will catch
- [x] 11c: Add test: create an App, send 100 `EventSubAgentDelta` events in a loop, verify that `sa.status` is updated for the last one (data still flows) but confirm the test doesn't hang or take excessive time (proxy for no per-event render overhead)
- [x] 11d: Add test: send `EventSubAgentStatus` with `text="done"`, verify render is called (by checking that `sa.done=true` is reflected in `subAgentDisplayLines()` output immediately after the event). Send `EventSubAgentStatus` with `text="tool: grep"`, verify `sa.status` is updated but the event doesn't require immediate rendering
