...
- [ ] 11a: In the `EventSubAgentDelta` handler (`agentui.go:571-582`), remove the `a.render()` call. The status snippet is still updated on `sa.status` — it just won't trigger an immediate render. The next `agentTickMsg` (every 50ms) will render it via the existing tick → `drainAgentEvents` → `render` path
- [ ] 11b: In the `EventSubAgentStatus` handler (`agentui.go:584`), move the `a.render()` call inside the `if event.Text == "done"` branch and the `else` branch that handles errors. For the non-done branch (tool start notifications at line 612-617), remove the `a.render()` call — these are frequent display-only updates that the tick will catch
- [ ] 11c: Add test: create an App, send 100 `EventSubAgentDelta` events in a loop, verify that `sa.status` is updated for the last one (data still flows) but confirm the test doesn't hang or take excessive time (proxy for no per-event render overhead)
- [ ] 11d: Add test: send `EventSubAgentStatus` with `text="done"`, verify render is called (by checking that `sa.done=true` is reflected in `subAgentDisplayLines()` output immediately after the event). Send `EventSubAgentStatus` with `text="tool: grep"`, verify `sa.status` is updated but the event doesn't require immediate rendering
