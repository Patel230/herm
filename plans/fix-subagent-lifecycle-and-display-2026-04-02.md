...
- [ ] 1a: Add a `HasPendingBackgroundAgents() bool` method to `SubAgentTool` that checks if any `bgAgentState` has `done == false`. Add it to the `BackgroundWaiter` interface
