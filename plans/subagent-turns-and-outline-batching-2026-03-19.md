...
- [ ] 1a: Refactor `SubAgentTool` struct — rename `model` to `mainModel`, keep `explorationModel`; update `NewSubAgentTool()` constructor signature and `main.go` call site to pass `modelID` as `mainModel` and `explorationModelID` as `explorationModel`
